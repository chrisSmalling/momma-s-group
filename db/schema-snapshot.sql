...
alter table public."places" add constraint "places_llm_verification_status_check" CHECK ((llm_verification_status = ANY (ARRAY['unverified'::text, 'verified'::text, 'needs_review'::text])));
...
AS $function$
  select case
    ...
    when length(normalize_for_evidence(p_evidence)) < 8 then false
    else position(
      normalize_for_evidence(p_evidence) in normalize_for_evidence(p_description)
    ) > 0
  end;
$function$
;
