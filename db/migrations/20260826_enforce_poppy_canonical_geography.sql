-- Poppy canonical geography + hard budget filtering
-- Applied to production Supabase project uiuibwufzhirpntdtqpj on 2026-08-26.

UPDATE public.events e
SET geography_tier = CASE
  WHEN lower(coalesce(e.address,'') || ' ' || coalesce(e.venue_name,'')) ~ 'tampa' THEN 'tampa'
  WHEN lower(coalesce(e.address,'') || ' ' || coalesce(e.venue_name,'')) ~ '(lutz|land o.? lakes|wesley chapel|new port richey|trinity|zephyrhills|hudson|odessa|san antonio|spring hill)' THEN 'pasco'
  WHEN lower(coalesce(e.address,'') || ' ' || coalesce(e.venue_name,'')) ~ '(plant city|brandon|riverview|valrico|thonotosassa|st[.]? petersburg|lakeland|clearwater|sarasota|bradenton|fort myers|jacksonville|orlando)' THEN 'far'
  ELSE 'unknown'
END
WHERE e.geography_tier IS NULL;

CREATE OR REPLACE VIEW public.poppy_recommendation_candidates AS
SELECT e.id, e.title, e.display_title, e.description, e.venue_name, e.venue_display,
       e.address, e.location_city, e.location_state, e.location_zip,
       e.lat, e.lng, e.location_latitude, e.location_longitude,
       e.starts_at, e.ends_at, e.age_min_months, e.age_max_months,
       e.age_band, e.age_tags, e.cost, e.source, e.source_url,
       e.registration_required, e.registration_url, e.is_outdoor,
       e.experience_type, e.weather_fit, e.is_kid_relevant,
       e.verification_tier, e.verification_score, e.content_review_status,
       e.last_verified_at, e.place_id, e.program_id
FROM public.events e
WHERE e.starts_at > now()
  AND coalesce(e.is_suppressed,false) = false
  AND e.status <> 'cancelled'
  AND e.content_review_status <> 'rejected'
  AND e.verification_tier IN ('trusted','high')
  AND e.geography_tier IN ('pasco','tampa');
