...
async function getWeatherAtLocation(location: { lat: number; lng: number }, eventStart: string): Promise<Weather | null> {
  try {
    ...
    const localHour = new Intl.DateTimeFormat("en-US", { timeZone: "America/New_York", year: "numeric", month: "2-digit", day: "2-digit", hour: "2-digit", hour12: false }).formatToParts(eventDate);
    const part = (name: string) => localHour.find((p) => p.type === name)?.value ?? "";
