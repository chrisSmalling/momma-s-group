...
import type { Mood, PoppyCandidate, RecommendationConstraints, Timeframe, TimeOfDay } from "./types";
...
export function easternDateKey(date:Date):string{const p=easternParts(date);return dateKey(p.year,p.month,p.day);}
...
// Phase A (honest unified candidate model): a place has no event-time to
// check against a calendar timeframe — its own season_start/season_end
// (both null = year-round) is what "available" means for it.
export function isPlaceInSeason(seasonStart:string|null,seasonEnd:string|null,now:Date):boolean{const today=easternDateKey(now);if(seasonStart&&today<seasonStart)return false;if(seasonEnd&&today>seasonEnd)return false;return true;}

const WEEKDAY_KEYS=["sun","mon","tue","wed","thu","fri","sat"] as const;
function parseHoursRange(trimmed:string):{openMin:number;closeMin:number}|null{const m=trimmed.match(/^(\d{1,2}):(\d{2})\s*-\s*(\d{1,2}):(\d{2})$/);if(!m)return null;return{openMin:Number(m[1])*60+Number(m[2]),closeMin:Number(m[3])*60+Number(m[4])};}
// "closed" is a real, known fact (absent day, or an explicit "closed"
// value) distinct from "unknown" (no hours object at all, or a value that
// doesn't parse — a data problem, not a fact about the place). Only a real
// "closed" should ever exclude a candidate; "unknown" must never be read as
// either open or closed.
export function placeOpenness(hours:PlaceHours|null,now:Date):"open"|"closed"|"unknown"{
  if(!hours)return"unknown";
  const p=easternParts(now);
  const raw=hours[WEEKDAY_KEYS[p.weekday]];
  if(raw===undefined)return"closed";
  const trimmed=raw.trim();
  if(!trimmed||trimmed.toLowerCase()==="closed")return"closed";
  const range=parseHoursRange(trimmed);
  if(!range)return"unknown";
  const nowMin=p.hour*60+p.minute;
  if(range.closeMin<range.openMin)return nowMin>=range.openMin||nowMin<range.closeMin?"open":"closed";
  return nowMin>=range.openMin&&nowMin<range.closeMin?"open":"closed";
}

export function eventWithinTimeframe(e:PoppyCandidate,timeframe:Timeframe,now:Date):boolean{
  if(e.kind==="place"){if(!isPlaceInSeason(e.season_start,e.season_end,now))return false;if(timeframe==="today"&&placeOpenness(e.hours,now)==="closed")return false;return true;}
...
export function eventMatchesTimeOfDay(e:PoppyCandidate,timeOfDay:TimeOfDay):boolean{if(timeOfDay==="any"||e.kind==="place")return true;const start=e.starts_at?easternMinutes(new Date(e.starts_at)):0;const end=e.ends_at?easternMinutes(new Date(e.ends_at)):start;const ws=timeOfDay==="morning"?360:timeOfDay==="afternoon"?720:1020;const we=timeOfDay==="morning"?720:timeOfDay==="afternoon"?1020:1260;if(end<start)return start<we||end>ws;return start<we&&end>ws;}
...
export interface FilteredPlace{place:Place;miles:number|null;}export interface FilteredEvent{event:PoppyCandidate;miles:number|null;}
...
export function filterEvents(events:PoppyCandidate[],c:RecommendationConstraints,origin:{lat:number;lng:number}|null,now:Date):{kept:FilteredEvent[];droppedCount:number}{let dropped=0;const kept:FilteredEvent[]=[];for(const event of events){if(event.status==="cancelled"||!eventWithinTimeframe(event,c.timeframe,now)||!eventMatchesTimeOfDay(event,c.timeOfDay)||!moodMatchesEvent(event,c.mood)||!passesIndoorHardFilter(event.is_outdoor,c)||!passesBudgetHardFilter(event.cost,c)){dropped++;continue;}const miles=milesBetween(origin,event);if(!passesDistanceHardFilter(miles,c)){dropped++;continue;}kept.push({event,miles});}return{kept,droppedCount:dropped};}
