...
function event(lat:number|null,lng:number|null,startsAt="2026-08-27T10:00:00-04:00",endsAt="2026-08-27T11:00:00-04:00"):FeedEvent{return{id:crypto.randomUUID(),title:"Activity",description:null,venue:"Venue",room_name:null,organizer:null,address:"1 Main St",lat,lng,location_latitude:null,location_longitude:null,starts_at:startsAt,ends_at:endsAt,time_precision:"exact",time_unknown:false,cost:"Free",is_free:true,age_tags:[],age_min_months:null,age_max_months:null,age_band:null,is_outdoor:false,what_to_bring:[],registration_required:false,registration_url:null,source:"test",source_id:null,source_url:null,content_status:"keep",geography_tier:"pasco",experience_type:"general",weather_fit:"indoor",place_id:null,program_id:null,proposed_by_group:null,metro_area:"pasco",status:"published",last_verified_at:null,added_by:null};}
...
describe("Eastern calendar semantics",()=>{
 it("uses the user's Eastern calendar day even when UTC has crossed midnight",()=>{
   const now=new Date("2026-08-28T02:30:00Z");
   expect(eventWithinTimeframe(event(28.2,-82.4,"2026-08-27T23:30:00-04:00"),"today",now)).toBe(true);
   expect(eventWithinTimeframe(event(28.2,-82.4,"2026-08-28T00:30:00-04:00"),"today",now)).toBe(false);
 });
 it("keeps tomorrow anchored to Eastern local dates",()=>{
   const now=new Date("2026-08-28T02:30:00Z");
