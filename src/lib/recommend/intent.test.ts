...
describe("parseIntent", () => {
  it("keeps local-first defaults", () => {
    expect(parseIntent("something fun")).toMatchObject({ mood: "all", indoor: "either", budget: "any", maxMiles: 20, maxPriceDollars: null, timeframe: "any", timeOfDay: "any", indoorExplicit: false });
  });
  it("recognizes explicit indoor, budget, distance, price, and timeframe constraints", () => {
    expect(parseIntent("Find something indoors under $20 within 10 miles today")).toMatchObject({ mood: "indoor", indoor: "indoor", indoorExplicit: true, budget: "budget", maxMiles: 10, maxPriceDollars: 20, timeframe: "today", timeOfDay: "any" });
  });
  it("recognizes near-me requests as a close local radius", () => { expect(parseIntent("What's near me right now?")).toMatchObject({ maxMiles: 8, timeframe: "today", timeOfDay: "any" }); });
  it("recognizes morning, afternoon, and evening windows", () => { expect(parseIntent("something fun this morning")).toMatchObject({ timeframe: "today", timeOfDay: "morning" }); expect(parseIntent("something fun after lunch")).toMatchObject({ timeframe: "any", timeOfDay: "afternoon" }); expect(parseIntent("something fun tonight")).toMatchObject({ timeframe: "today", timeOfDay: "evening" }); });
