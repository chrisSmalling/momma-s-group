...
export default function TodayFeed({
  ...
  return (
    <>
      ...
      {hasActiveGroup && friendSummary.length > 0 && (
        <section
          className="mb-5 rounded-2xl border border-rose-200 bg-rose-50/70 p-4"
          aria-label="Your friends are going"
        >
          <div className="flex items-start justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-xs text-zinc-600">
                See where moms in your group are actually going today.
              </p>
            </div>
            <button
              type="button"
              onClick={() => setSelectedFilter("friends")}
              className="shrink-0 rounded-full bg-white px-3 py-2 text-xs font-bold text-rose-700 shadow-sm"
            >
              See all
            </button>
          </div>
          ...
          <div className="mt-3 space-y-2">
            {friendSummary.map((b) => {
              const friends = b.attendees.filter(
                (a) =>
                  a.user_id !== currentUserId &&
                  activeGroupMemberIds.includes(a.user_id) &&
                  a.status === "going"
              ...
              const names = friends
                .slice(0, 2)
                .map((f) => f.display_name.split(/\s+/)[0])
                .join(" + ");
              const extra = Math.max(0, friends.length - 2);
                  ...
                  <div className="flex items-center justify-between gap-3">
                    <div className="min-w-0">
                      ...
                      <div className="mt-1 text-xs font-semibold text-zinc-500">
                        {names}
                        {extra > 0 ? ` + ${extra}` : ""} {friends.length === 1 ? "is" : "are"} going
      ...
      <div
        className="-mx-4 mb-5 overflow-x-auto px-4 pb-1"
        role="tablist"
      ...
      {visible.length === 0 ? (
        ...
        <>
          <div className="mb-3 flex items-center justify-between gap-3">
            <div>
              <h3 className="font-display text-lg font-bold text-zinc-950">Today&apos;s best bets</h3>
              <p className="text-xs text-zinc-500">A short list first — more only when you ask for it.</p>
            </div>
