...
const miles = (a: Origin, b: { lat: number | null; lng: number | null }) => {
...
function matchesPlace(p: Place, mood: Mood) {
  if (mood === "all") return true; const t = placeText(p);
...
function matchesEvent(e: Event, mood: Mood) {
  if (mood === "all") return true; const t = eventText(e);
...
const ageScore = (min: number | null, max: number | null, age: number | null) => { if (age == null) return 0; const lo = min ?? 0, hi = max ?? 144; return age >= lo && age <= hi ? 40 : age >= lo - 6 && age <= hi + 6 ? 12 : -25; };
...
export default function ExplorerAssistant({ places, events, childAgeMonths, homeLat, homeLng, weather = null }: Props) {
  ...
  const results = useMemo<Result[]>(() => {
    ...
    }).filter(r => (r.distance == null || r.distance <= parsed.maxMiles) && (parsed.maxMinutes == null || (r.item.starts_at && (new Date(r.item.starts_at).getTime() - Date.now()) >= 0 && (new Date(r.item.starts_at).getTime() - Date.now()) / 60000 <= parsed.maxMinutes)));
  ...
  return <section className="overflow-hidden rounded-3xl border border-rose-100 bg-gradient-to-br from-rose-50 via-white to-amber-50 shadow-sm">
    <div className="p-5 sm:p-6">
      <div className="text-xs font-bold uppercase tracking-[0.16em] text-rose-600">Your Momma&apos;s Meetup Explorer</div><h2 className="mt-1 font-display text-2xl font-bold tracking-tight text-zinc-950">Tell me what you need today.</h2><p className="mt-2 text-sm leading-6 text-zinc-600">Talk to me like a friend. I&apos;ll look across places and what&apos;s actually happening today.</p>
      ...
      <div className="mt-3 flex flex-wrap items-center gap-2"><button type="button" onClick={locate} className={`rounded-full border px-3 py-2 text-xs font-bold ${usingCurrent ? "border-rose-500 bg-rose-50 text-rose-700" : "border-zinc-200 bg-white text-zinc-700"}`}>📍 Find near me</button>{homeLat != null && homeLng != null && <button type="button" onClick={() => { setOrigin({ lat: homeLat, lng: homeLng }); setUsingCurrent(false); setMessage(null); }} className="rounded-full border border-zinc-200 bg-white px-3 py-2 text-xs font-bold text-zinc-700">🏠 Use home</button>}<span className="text-xs text-zinc-500">{intro}</span></div>{message && <div className="mt-3 rounded-xl bg-white/80 px-3 py-2 text-xs text-zinc-600">{message}</div>}
    </div>
    <div className="border-t border-rose-100 bg-white/80 p-5 sm:p-6">
      <div className="flex items-end justify-between gap-3"><div><h3 className="font-display text-lg font-bold text-zinc-950">✨ Here&apos;s what I&apos;d pick</h3><p className="text-xs text-zinc-500">A few strong choices, not an endless scroll.</p></div>{results.length > 0 && <button type="button" onClick={() => setPlanOpen(!planOpen)} className="rounded-full bg-zinc-900 px-3 py-2 text-xs font-bold text-white">Build my day</button>}</div>
      {results.length === 0 ? <p className="mt-4 rounded-2xl bg-zinc-50 p-4 text-sm text-zinc-600">I don&apos;t have a verified match for those constraints yet. Try widening the distance or changing the vibe.</p> : <div className="mt-4 grid gap-3">{results.map((r, i) => <article key={r.item.id} className="rounded-2xl border border-zinc-200 bg-white p-4 shadow-sm"><div className="flex items-start justify-between gap-3"><div><div className="text-xs font-bold uppercase tracking-wide text-rose-600">{i === 0 ? "Best match" : r.type === "event" ? "Happening today" : "Also good"}</div><h4 className="mt-1 font-bold text-zinc-950">{r.type === "place" ? r.item.name : r.item.title}</h4><p className="mt-1 text-xs text-zinc-500">{r.distance != null ? `${Math.round(r.distance)} mi away` : "Distance unavailable"}{r.type === "event" ? ` · ${new Date(r.item.starts_at).toLocaleTimeString([], { hour: "numeric", minute: "2-digit" })}` : r.item.price_note ? ` · ${r.item.price_note}` : ""}</p></div><span className="text-xl">{r.type === "event" ? "📅" : r.item.is_outdoor ? "🌳" : "🏠"}</span></div><div className="mt-3 flex flex-wrap gap-1.5">{childAgeMonths != null && <span className="rounded-full bg-rose-50 px-2 py-1 text-[11px] font-semibold text-rose-700">Toddler fit</span>}{r.type === "event" && <span className="rounded-full bg-amber-50 px-2 py-1 text-[11px] font-semibold text-amber-700">Today</span>}{r.type === "place" && r.item.restrooms && <span className="rounded-full bg-zinc-100 px-2 py-1 text-[11px]">Restrooms</span>}{r.type === "place" && r.item.stroller_accessible && <span className="rounded-full bg-zinc-100 px-2 py-1 text-[11px]">Stroller</span>}</div>{r.type === "place" && r.item.toddler_notes && <p className="mt-3 text-sm leading-5 text-zinc-600"><b>Why:</b> {r.item.toddler_notes}</p>}{r.type === "event" && r.item.description && <p className="mt-3 line-clamp-2 text-sm leading-5 text-zinc-600">{r.item.description}</p>}</article>)}</div>}
