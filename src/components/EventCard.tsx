...
function CancelledPill() {
  return <span className="shrink-0 rounded-full bg-zinc-200 px-2.5 py-0.5 text-xs font-semibold text-zinc-600">Cancelled</span>;
}
      ...
      <div className="flex gap-3">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-start justify-between gap-2">
            <h3 className={cancelled ? "truncate text-lg font-bold text-zinc-400 line-through" : "truncate text-lg font-bold text-zinc-900"}>{event.title}</h3>
            {cancelled ? <CancelledPill /> : <CostPill cost={event.cost} />}
          </div>
          <p className="text-sm font-semibold text-zinc-600">{formatTime(event)}</p>
          {event.venue && <p className="truncate text-sm text-zinc-500">{event.venue}{event.room_name ? ` · ${event.room_name}` : ""}</p>}
          {(event.age_tags.length > 0 || goodAgeFit) && <div className="mt-1.5 flex flex-wrap items-center gap-1.5">
            {event.age_tags.length > 0 && <span className="inline-block rounded-full bg-rose-50 px-2.5 py-0.5 text-xs font-medium text-rose-600">{event.age_tags.join(" · ")}</span>}
