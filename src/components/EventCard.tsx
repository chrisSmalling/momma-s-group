        ...
        <span
          ...
          className={
            chip.tone === "sage"
              ...
              : "inline-flex items-center rounded-full bg-amber-50 px-2.5 py-1 text-xs font-semibold text-amber-800"
          }
        >
          {chip.label}
        </span>
  ...
  return (
    <EventCardShell eventId={event.id} currentStatus={currentStatus} currentNote={currentNote} disabled={cancelled} duringNap={duringNap}>
      <Link
        ...
        className="group block rounded-xl -m-1 p-1 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-rose-500"
        ...
        <div className="flex gap-3">
          ...
          <div className="min-w-0 flex-1">
            <div className="flex items-start justify-between gap-2">
              <h3 className={cancelled ? "font-display text-xl font-bold leading-tight text-zinc-400 line-through" : "font-display text-xl font-bold leading-tight text-zinc-900 group-hover:text-rose-700"}>
                {event.title}
              </h3>
            ...
            {event.age_tags.length > 0 && (
              <div className="mt-1.5 flex flex-wrap items-center gap-1.5">
                <span className="inline-block rounded-full bg-zinc-100 px-2.5 py-0.5 text-xs font-medium text-zinc-600">{event.age_tags.join(" · ")}</span>
              </div>
            )}
            {!cancelled && (
              <FitChips
                goodAgeFit={goodAgeFit}
            ...
            {proposedBy && (
              <p className="mt-1.5 text-xs italic text-zinc-400">
                Proposed by {proposedBy.user_id === currentUserId ? "you" : proposedBy.display_name}
              </p>
            )}
          </div>
        </div>
