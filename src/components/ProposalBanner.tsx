...
export default function ProposalBanner({ proposals }: { proposals: Proposal[] }) {
  ...
  return (
    <div className="mb-6 rounded-2xl border border-amber-200 bg-amber-50 p-4 shadow-sm" role="status" aria-live="polite">
      <div className="flex items-start gap-3">
        <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-amber-100 text-lg" aria-hidden="true">📣</div>
        <div className="min-w-0 flex-1">
          <p className="text-sm font-bold text-amber-950">New meetup proposal{count > 1 ? "s" : ""}</p>
