...
function placeText(p: Place) { return `${p.name} ${p.description ?? ""} ${p.toddler_notes ?? ""} ${p.price_note ?? ""}`.toLowerCase(); }
function eventText(e: Event) { return `${e.title} ${e.description ?? ""} ${e.venue_name ?? ""} ${e.cost ?? ""}`.toLowerCase(); }
function ageScore(min: number | null, max: number | null, age: number | null) {
  if (age == null) return 0;
  const lo = min ?? 0, hi = max ?? 144;
...
function infer(q: string) {
  ...
  const minutes = s.match(/(?:for|about|around)?\s*(\d+)\s*(minute|minutes|min|mins)/);
  const maxMinutes = hours ? Math.round(Number(hours[1]) * 60) : minutes ? Number(minutes[1]) : null;
  const budget = /free|cheap|low cost|budget|don't want to spend|no money/.test(s);
  return { mood, budget, easy: /tired|exhausted|easy|low key|low-key|simple/.test(s), now: /today|now|right now|already out|we're out/.test(s), maxMiles: /5\s*(?:mile|mi)/.test(s) ? 5 : /10\s*(?:mile|mi)/.test(s) ? 10 : /20\s*(?:mile|mi)/.test(s) ? 20 : 30, maxMinutes };
}
...
function matchesEvent(e: Event, mood: Mood) {
  ...
  if (mood === "outdoor") return e.is_outdoor === true || /park|playground|nature|farm|zoo|garden/.test(t);
...
export default function ExplorerAssistantV2({ places, events, childAgeMonths, homeLat, homeLng, weather = null }: Props) {
  ...
  const parsed = useMemo(() => infer(submitted), [submitted]);
  const effectiveMood = mood === "all" ? parsed.mood : mood;

  const results = useMemo<Result[]>(() => {
    const placeResults: Result[] = places.filter(p => matchesPlace(p, effectiveMood)).map(place => {
      const distance = miles(origin, place);
      let score = ageScore(place.age_min_months, place.age_max_months, childAgeMonths) + weatherScore(weather, place.is_outdoor);
      score += effectiveMood !== "all" ? 35 : 0;
      if (place.toddler_notes) score += 8;
      if (place.restrooms) score += 3;
    ...
    const eventResults: Result[] = events.filter(e => e.status === "published" && e.is_kid_relevant && matchesEvent(e, effectiveMood)).map(event => {
      const distance = miles(origin, event);
      const minutesUntil = (new Date(event.starts_at).getTime() - Date.now()) / 60000;
      let score = ageScore(event.age_min_months, event.age_max_months, childAgeMonths) + weatherScore(weather, event.is_outdoor);
      if (effectiveMood !== "all") score += 35;
      const t = eventText(event);
      if (parsed.budget && /free|no cost|\$0/.test(t)) score += 15;
      if (parsed.now) score += 10;
      if (parsed.maxMinutes != null && minutesUntil >= 0 && minutesUntil <= parsed.maxMinutes) score += 15;
      if (distance != null) score += Math.max(0, 25 - distance * 1.1);
      return { type: "event", item: event, distance, score };
    }).filter(r => (r.distance == null || r.distance <= parsed.maxMiles) && (parsed.maxMinutes == null || r.item && ((new Date(r.item.starts_at).getTime() - Date.now()) / 60000) >= 0));
  ...
  return <section className="overflow-hidden rounded-3xl border border-rose-100 bg-gradient-to-br from-rose-50 via-white to-amber-50 shadow-sm">
    <div className="p-5 sm:p-6">
      <div className="text-xs font-bold uppercase tracking-[0.16em] text-rose-600">Your Momma&apos;s Meetup Explorer</div>
      <h2 className="mt-1 font-display text-2xl font-bold tracking-tight text-zinc-950">Tell me what you need today.</h2>
      <p className="mt-2 text-sm leading-6 text-zinc-600">Talk to me like a friend. I&apos;ll look across places and what&apos;s actually happening today.</p>
      ...
      <div className="mt-4 flex flex-wrap gap-2">{chips.map(c => <button key={c.mood} type="button" onClick={() => setMood(mood === c.mood ? "all" : c.mood)} className={`rounded-full border px-3 py-2 text-sm font-semibold transition ${mood === c.mood ? "border-rose-500 bg-rose-600 text-white" : "border-zinc-200 bg-white text-zinc-700 hover:border-rose-200"}`}>{c.icon} {c.label}</button>)}</div>
      ...
      <div className="mt-3 flex flex-wrap items-center gap-2"><button type="button" onClick={locate} className={`rounded-full border px-3 py-2 text-xs font-bold ${usingCurrent ? "border-rose-500 bg-rose-50 text-rose-700" : "border-zinc-200 bg-white text-zinc-700"}`}>📍 Find near me</button>{homeLat != null && homeLng != null && <button type="button" onClick={() => { setOrigin({ lat: homeLat, lng: homeLng }); setUsingCurrent(false); setMessage(null); }} className="rounded-full border border-zinc-200 bg-white px-3 py-2 text-xs font-bold text-zinc-700">🏠 Use home</button>}<span className="text-xs text-zinc-500">{submitted ? `Matched against verified places and today’s kid-relevant events${usingCurrent ? " near you" : ""}.` : childAgeMonths != null ? `I’ll keep your ${Math.floor(childAgeMonths / 12)}-year-old in mind.` : "Tell me what you need and I’ll narrow it down."}</span></div>
      {message && <div className="mt-3 rounded-xl bg-white/80 px-3 py-2 text-xs text-zinc-600">{message}</div>}
    </div>
    <div className="border-t border-rose-100 bg-white/80 p-5 sm:p-6"><div><h3 className="font-display text-lg font-bold text-zinc-950">✨ Here&apos;s what I&apos;d pick</h3><p className="text-xs text-zinc-500">A few strong choices, not an endless scroll.</p></div>
      {results.length === 0 ? <p className="mt-4 rounded-2xl bg-zinc-50 p-4 text-sm text-zinc-600">I don&apos;t have a verified match for those constraints yet. Try widening the distance or changing the vibe.</p> : <div className="mt-4 grid gap-3">{results.map((r, i) => <article key={`${r.type}-${r.item.id}`} className="rounded-2xl border border-zinc-200 bg-white p-4 shadow-sm"><div className="flex items-start justify-between gap-3"><div><div className="text-xs font-bold uppercase tracking-wide text-rose-600">{i === 0 ? "Best match" : r.type === "event" ? "Happening today" : "Also good"}</div><h4 className="mt-1 font-bold text-zinc-950">{r.item.name ?? r.item.title}</h4><p className="mt-1 text-xs text-zinc-500">{r.distance != null ? `${Math.round(r.distance)} mi away` : "Distance unavailable"}{r.type === "event" ? ` · ${new Date(r.item.starts_at).toLocaleTimeString([], { hour: "numeric", minute: "2-digit" })}` : (r.item as Place).price_note ? ` · ${(r.item as Place).price_note}` : ""}</p></div><span className="text-xl">{r.type === "event" ? "📅" : (r.item as Place).is_outdoor ? "🌳" : "🏠"}</span></div><div className="mt-3 flex flex-wrap gap-1.5">{childAgeMonths != null && <span className="rounded-full bg-rose-50 px-2 py-1 text-[11px] font-semibold text-rose-700">Age fit</span>}{r.type === "event" && <span className="rounded-full bg-amber-50 px-2 py-1 text-[11px] font-semibold text-amber-700">Today</span>}{r.type === "place" && (r.item as Place).restrooms && <span className="rounded-full bg-zinc-100 px-2 py-1 text-[11px]">Restrooms</span>}{r.type === "place" && (r.item as Place).stroller_accessible && <span className="rounded-full bg-zinc-100 px-2 py-1 text-[11px]">Stroller</span>}</div>{r.type === "place" && (r.item as Place).toddler_notes && <p className="mt-3 text-sm leading-5 text-zinc-600"><b>Why:</b> {(r.item as Place).toddler_notes}</p>}{r.type === "event" && (r.item as Event).description && <p className="mt-3 line-clamp-2 text-sm leading-5 text-zinc-600">{(r.item as Event).description}</p>}</article>)}</div>}
    </div>
  </section>;
}
