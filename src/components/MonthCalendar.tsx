...
type Filter = "all" | "free" | "indoor" | "outdoor" | "mine" | "age_fit";
type Mode = "all" | "mine" | "group";
type View = "agenda" | "grid";

// No separate "I'm going" chip — the "My plans" mode tab above already
// covers that (and more: it also includes Maybe and events I proposed),
// so a same-meaning filter chip next to it was confusing two different
// control types for nearly the same thing.
const FILTERS: { id: Filter; label: string }[] = [{ id: "all", label: "All" }, { id: "free", label: "Free" }, { id: "indoor", label: "Indoor" }, { id: "outdoor", label: "Outdoor" }, { id: "mine", label: "Group proposals" }, { id: "age_fit", label: "Good age fit" }];
...
export default function MonthCalendar({ year, month0, events, prevHref, nextHref, cards, myRsvpByEvent, activeGroupId, childAgeMonths, plansHref, groupEventIds, currentUserId }: { year: number; month0: number; events: FeedEvent[]; prevHref: string; nextHref: string; cards: Record<string, ReactNode>; myRsvpByEvent: Record<string, RsvpStatus>; activeGroupId: string | null; childAgeMonths: number | null; plansHref: string; groupEventIds: string[]; currentUserId: string }) {
  ...
  const filtered = useMemo(() => { const q = query.trim().toLowerCase(); return scoped.filter((e) => { if (!isInEtMonth(e.starts_at, year, month0)) return false; if (q && !`${e.title} ${e.venue ?? ""} ${e.address ?? ""}`.toLowerCase().includes(q)) return false; if (filter === "free" && e.is_free !== true) return false; if (filter === "indoor" && e.is_outdoor === true) return false; if (filter === "outdoor" && e.is_outdoor !== true) return false; if (filter === "mine" && e.proposed_by_group == null) return false; if (filter === "age_fit" && !isGoodAgeFit(childAgeMonths, e.age_min_months, e.age_max_months)) return false; return true; }).sort((a, b) => new Date(a.starts_at).getTime() - new Date(b.starts_at).getTime()); }, [scoped, query, filter, childAgeMonths, year, month0]);
