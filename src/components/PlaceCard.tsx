...
export default function PlaceCard({ place, groupId, groupName, currentUserId, tips, distance, childAgeMonths, weather }: {
  place: Place;
  groupId: string | null;
  ...
  return (
    <div className="rounded-2xl border border-zinc-200 bg-white p-4">
      <div className="flex items-start justify-between gap-2">
        <h3 className="min-w-0 text-lg font-bold text-zinc-900">{place.name}</h3>
        {distance !== undefined && <span className="shrink-0 text-xs text-zinc-400">{distance.driveMinutes !== undefined ? formatDriveTime(distance.driveMinutes) : formatDistance(distance.km)}</span>}
      </div>
