...
export async function POST(request: Request) {
  ...
  if (originMode === "current") {
    const supplied = body.origin as Record<string, unknown> | undefined;
    if (supplied && typeof supplied.lat === "number" && typeof supplied.lng === "number" && Number.isFinite(supplied.lat) && Number.isFinite(supplied.lng)) origin = { lat: supplied.lat, lng: supplied.lng };
  }
  ...
  const ranked = filtered.kept.map(({ place, miles }) => ({ place, miles, score: scorePlace(place, miles, constraints, profile) }))
  ...
  const candidates: RecommendationCandidate[] = ranked.map(({ place, miles, score }) => {
    ...
    const interest = profile.childInterests.find(i => {
      const tags: Record<string, string[]> = { playgrounds: ["playground"], water: ["water_play"], adventure: ["active_play", "outdoor"], sports: ["active_play", "playground"], animals: ["animals"], books: ["storytime"], arts_and_crafts: ["arts_learning"], science: ["arts_learning"], music: ["arts_learning"] };
      return (tags[i] ?? []).some(tag => place.category_tags.includes(tag));
    });
    ...
    return {
      type: "place", id: place.id, title: place.name, description: place.toddler_notes ?? place.description, address: place.address,
      distanceMiles: miles, driveMinutes: null, distanceLabel: miles == null ? null : `~${miles < 10 ? miles.toFixed(1) : Math.round(miles)} mi away`, startsAt: null, endsAt: null,
      price: place.price_note, isFree: isFreeCost(place.price_note), isOutdoor: place.is_outdoor, ageMinMonths: place.age_min_months, ageMaxMonths: place.age_max_months,
      goodAgeFit: fit, reason: reasons.length ? `Good option${reasons.length ? ` — ${reasons.join(" · ")}` : ""}.` : "A family-friendly option.", href: `/places/${place.id}`,
      lastVerifiedAt: place.last_verified_at, score, whatToBring: place.what_to_bring ?? [], strollerAccessible: place.stroller_accessible, changingTable: place.has_changing_table,
      nursingFriendly: place.nursing_friendly, parkingNotes: place.parking_notes, typicalCrowdNote: place.typical_crowd_note, bestTimeNote: place.best_time_note, registrationRequired: false,
    };
  });
