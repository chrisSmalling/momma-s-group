...
import type { IndoorPreference, Mood, PoppyProfile, RecommendationConstraints, RecommendationResult } from "@/lib/recommend/types";
...
function toProfile(row: Record<string, unknown> | null): PoppyProfile {
  ...
  return {
    ...
    preferredCategories: arr(row?.preferred_categories), preferredPlaceTypes: arr(row?.preferred_place_types),
    indoorPreference: indoorPref === "indoor" || indoorPref === "outdoor" ? indoorPref : "either", maxDistanceMiles: num(row?.max_distance_miles),
    familyBudgetNote: str(row?.family_budget_note), napStart: str(row?.nap_start), napEnd: str(row?.nap_end), homeLat: num(row?.home_lat), homeLng: num(row?.home_lng),
  };
}

type PoppyCandidateRow = {
  id: string; title: string | null; display_title: string | null; description: string | null; venue_name: string | null; venue_display: string | null;
  address: string | null; location_city: string | null; location_state: string | null; location_zip: string | null; lat: number | null; lng: number | null;
  location_latitude: number | null; location_longitude: number | null; starts_at: string; ends_at: string | null; age_min_months: number | null;
  age_max_months: number | null; age_band: string | null; age_tags: string[] | null; cost: string | null; source: string | null; source_url: string | null;
  registration_required: boolean; registration_url: string | null; is_outdoor: boolean; experience_type: string | null; weather_fit: string | null;
  verification_tier: string | null; content_review_status: string | null; last_verified_at: string | null; place_id: string | null; program_id: string | null;
};

type PlaceTrustRow = { id: string; stroller_accessible: boolean | null; has_changing_table: boolean | null; nursing_friendly: boolean | null; parking_notes: string | null; typical_crowd_note: string | null; best_time_note: string | null; what_to_bring: string[] | null };
...
function candidateToFeedEvent(row: PoppyCandidateRow, place?: PlaceTrustRow, tips: string[] = []): FeedEvent {
  return {
    ...
    place_id: row.place_id, program_id: row.program_id, proposed_by_group: null, metro_area: row.location_city ?? "", status: "published", last_verified_at: row.last_verified_at, added_by: null,
    trust_stroller_accessible: place?.stroller_accessible ?? null, trust_changing_table: place?.has_changing_table ?? null, trust_nursing_friendly: place?.nursing_friendly ?? null,
    trust_parking_notes: place?.parking_notes ?? null, trust_typical_crowd_note: place?.typical_crowd_note ?? null, trust_best_time_note: place?.best_time_note ?? null,
    trust_community_tips: tips,
  } as FeedEvent;
}
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (constraints.maxMiles == null && profile.maxDistanceMiles != null) constraints.maxMiles = profile.maxDistanceMiles;
    const now = new Date(); const fallbackRequestId = randomUUID(); const cacheKey = buildCacheKey(user.id, constraints, profile, origin, now);

    const { data: cached, error: cacheReadError } = await supabase.from("recommendation_response_cache").select("response, expires_at").eq("request_hash", cacheKey).eq("user_id", user.id).gt("expires_at", now.toISOString()).maybeSingle();
    ...
    const rows = (candidateRows ?? []) as PoppyCandidateRow[];
    const placeIds = [...new Set(rows.map((r) => r.place_id).filter((id): id is string => Boolean(id)))];
    const eventIds = rows.map((r) => r.id);
    const placeMap = new Map<string, PlaceTrustRow>();
    const tipsByTarget = new Map<string, string[]>();

    if (placeIds.length > 0) {
      const { data: placeRows, error } = await supabase.from("places").select("id, stroller_accessible, has_changing_table, nursing_friendly, parking_notes, typical_crowd_note, best_time_note, what_to_bring").in("id", placeIds);
      if (error) console.error("[poppy] place trust enrichment failed", error.message);
      for (const place of (placeRows ?? []) as PlaceTrustRow[]) placeMap.set(place.id, place);
      const { data: tips, error: tipsError } = await supabase.from("place_tips").select("place_id, event_id, body").in("place_id", placeIds).limit(300);
      if (tipsError) console.error("[poppy] community tip enrichment failed", tipsError.message);
      for (const tip of (tips ?? []) as TipRow[]) { if (!tip.body?.trim()) continue; const key = tip.place_id ?? ""; tipsByTarget.set(key, [...(tipsByTarget.get(key) ?? []), tip.body.trim()].slice(0, 3)); }
    }
    if (eventIds.length > 0) {
      const { data: eventTips, error: eventTipsError } = await supabase.from("place_tips").select("place_id, event_id, body").in("event_id", eventIds).limit(300);
      if (eventTipsError) console.error("[poppy] event community tip enrichment failed", eventTipsError.message);
      for (const tip of (eventTips ?? []) as TipRow[]) { if (!tip.body?.trim()) continue; const key = tip.event_id ?? ""; tipsByTarget.set(key, [...(tipsByTarget.get(key) ?? []), tip.body.trim()].slice(0, 3)); }
    }

    const eventList = rows.map((row) => candidateToFeedEvent(row, row.place_id ? placeMap.get(row.place_id) : undefined, tipsByTarget.get(row.id) ?? (row.place_id ? tipsByTarget.get(row.place_id) ?? [] : [])));
    let weather: WeatherContext | null = null;
    if (origin) { try { weather = await getWeatherContext(origin.lat, origin.lng); } catch { weather = null; } }
