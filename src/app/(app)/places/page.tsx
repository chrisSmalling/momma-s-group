...
export default async function PlacesPage() {
  ...
  const [{ data: places }, { data: groups }, { data: profile }] = await Promise.all([
  ...
  const { data: tips } = activeGroupId && placeIds.length
    ? await supabase.from("place_tips").select("*").eq("group_id", activeGroupId).in("place_id", placeIds)
    : { data: [] };
  ...
  for (const tip of tipRows) {
    ...
    const list = tipsByPlace[tip.place_id] ?? [];
    list.push({ ...tip, display_name: nameById.get(tip.user_id) ?? "Someone" });
    tipsByPlace[tip.place_id] = list;
  }
