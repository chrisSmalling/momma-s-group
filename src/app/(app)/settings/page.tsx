...
export default async function SettingsPage(props: PageProps<"/settings">) {
  ...
  const { data: profile } = await supabase
    .from("profiles")
    .select("nap_start, nap_end, child_age_months, home_address, home_lat, home_lng")
    .eq("id", user.id)
    .maybeSingle();
  ...
  return (
    <div className="flex flex-1 flex-col items-center px-4 py-10">
      <div className="w-full max-w-md">
        ...
        <p className="mb-4 text-sm text-zinc-500">Enter the address you want Momma&apos;s Meetup to use as your home base. We&apos;ll use it to calculate real driving distance and time to events and places. You never need to enter latitude or longitude.</p>
        ...
        <form action={updateHomeLocation} className="flex flex-col gap-4">
          <label className="flex flex-col gap-1 text-sm text-zinc-600">Home address<input type="text" name="home_address" defaultValue={profile?.home_address ?? ""} autoComplete="street-address" placeholder="123 Main St, Wesley Chapel, FL 33543" className="rounded-md border border-zinc-300 px-3 py-2 text-sm text-zinc-900 outline-none focus:border-zinc-500" /></label>
          <p className="text-xs text-zinc-400">Your address is the source of truth. Coordinates, when available, are stored only as an internal routing cache.</p>
