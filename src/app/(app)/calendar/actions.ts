...
// Persistence layer for EventCardShell's optimistic UI: the caller already
// knows the intended end state (including null to clear), so this just
// writes it — no server-side toggle detection needed.
export async function rsvp(
  ...
  if (trimmedNote && trimmedNote.length > 200) {
    return { error: "Note is too long (200 characters max)" };
  }
  ...
  const { error } = status
    ? await supabase
        .from("rsvps")
        .upsert({ event_id: eventId, user_id: user.id, status, note: trimmedNote })
    : await supabase
        .from("rsvps")
        .delete()
        .eq("event_id", eventId)
