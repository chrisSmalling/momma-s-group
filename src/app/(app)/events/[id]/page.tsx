...
import type { EventComment, FeedEvent, Place, PlaceTip, RsvpStatus } from "@/types";
...
export default async function EventDetailPage({ params }: PageProps<"/events/[id]">) {
  ...
  const profileIds = [...new Set([
    ...
    ...groupRsvps.map((r) => r.user_id),
    ...(commentsResult.data ?? []).map((c) => c.user_id),
    ...(tipsResult.data ?? []).map((t) => t.user_id),
  ])];
  const { data: profiles } = profileIds.length
    ? await supabase.from("profiles").select("id, display_name, avatar_color").in("id", profileIds)
    : { data: [] };
  const profileById = new Map((profiles ?? []).map((p) => [p.id, p]));
  const roster = Object.fromEntries(activeGroupMemberIds.map((memberId) => [memberId, {
    display_name: profileById.get(memberId)?.display_name ?? "Someone",
    avatar_color: profileById.get(memberId)?.avatar_color ?? "#C0356E",
  }]));
  const attendees: Attendee[] = groupRsvps.map((r) => ({
    user_id: r.user_id,
    status: r.status as RsvpStatus,
    display_name: profileById.get(r.user_id)?.display_name ?? "Someone",
    avatar_color: profileById.get(r.user_id)?.avatar_color ?? "#C0356E",
  }));
  const comments: CommentDisplay[] = ((commentsResult.data ?? []) as EventComment[]).map((comment) => ({
    ...comment,
    display_name: profileById.get(comment.user_id)?.display_name ?? "Someone",
  }));
  const tips: TipDisplay[] = ((tipsResult.data ?? []) as PlaceTip[]).map((tip) => ({
    ...tip,
    display_name: profileById.get(tip.user_id)?.display_name ?? "Someone",
  }));
  ...
  return (
    <div className="mx-auto w-full max-w-2xl px-4 pb-6 pt-1">
      ...
      <div className="mb-4">
        <Link href="/today" className="inline-flex min-h-11 items-center gap-2 rounded-full px-2 text-sm font-bold text-rose-700 hover:bg-rose-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-rose-500">
      ...
      <EventCardShell eventId={feedEvent.id} currentStatus={currentRsvp} currentNote={currentNote} disabled={cancelled}>
        <header>
          ...
          <h1 className={`mt-4 font-display text-3xl font-bold leading-tight sm:text-4xl ${cancelled ? "text-zinc-400 line-through" : "text-zinc-950"}`}>
            {feedEvent.title}
          </h1>
        ...
        <section className="mt-6 grid grid-cols-2 gap-2" aria-label="Event essentials">
          <div className="rounded-2xl bg-rose-50 px-4 py-3">
            <p className="text-[11px] font-bold uppercase tracking-wider text-rose-700">When</p>
            <p className="mt-0.5 text-sm font-bold text-zinc-900">{formatDateTime(feedEvent.starts_at)}</p>
            {end && <p className="text-xs font-medium text-zinc-500">until {end}</p>}
          </div>
        ...
        {(feedEvent.description || place?.description || place?.toddler_notes) && (
          <section className="mt-6">
            <h2 className="font-display text-2xl font-bold text-zinc-900">What to expect</h2>
            {feedEvent.description && <p className="mt-2 whitespace-pre-line text-sm leading-6 text-zinc-700">{feedEvent.description}</p>}
            {place?.description && !feedEvent.description && <p className="mt-2 whitespace-pre-line text-sm leading-6 text-zinc-700">{place.description}</p>}
            {place?.toddler_notes && <p className="mt-3 rounded-2xl bg-rose-50 px-4 py-3 text-sm leading-6 text-zinc-700"><span className="font-bold text-rose-800">Momma note:</span> {place.toddler_notes}</p>}
          </section>
        )}
        ...
        <section className="mt-6">
          ...
          <div className="mt-3 flex flex-col gap-3">
            ...
            {feedEvent.registration_required && feedEvent.registration_url && (
              <a href={feedEvent.registration_url} target="_blank" rel="noopener noreferrer" className="inline-flex min-h-11 items-center justify-center rounded-xl bg-rose-600 px-4 text-sm font-bold text-white shadow-sm hover:bg-rose-700">Register for this event ↗</a>
            )}
            ...
            {(place?.website || place?.booking_url || feedEvent.source_url) && (
              <div className="flex flex-wrap gap-3 pt-1">
                {place?.booking_url && <a href={place.booking_url} target="_blank" rel="noopener noreferrer" className="text-sm font-bold text-rose-700 underline underline-offset-2">Booking</a>}
                {place?.website && <a href={place.website} target="_blank" rel="noopener noreferrer" className="text-sm font-bold text-rose-700 underline underline-offset-2">Venue website</a>}
                {feedEvent.source_url && <a href={feedEvent.source_url} target="_blank" rel="noopener noreferrer" className="text-sm font-bold text-rose-700 underline underline-offset-2">Event source</a>}
              </div>
            )}
          </div>
        </section>
        ...
        <section className="mt-7 rounded-2xl border border-zinc-200 bg-white p-4">
          <div className="flex items-start justify-between gap-3">
            <div>
              <h2 className="font-display text-2xl font-bold text-zinc-900">Who&apos;s going?</h2>
              <p className="mt-0.5 text-sm text-zinc-500">See if your group is already making plans.</p>
            </div>
            <span className="rounded-full bg-rose-50 px-3 py-1 text-xs font-bold text-rose-700">{attendees.filter((a) => a.status === "going").length} going</span>
          </div>
          <div className="mt-4">
            <LiveAttendees eventId={feedEvent.id} currentUserId={user.id} hasActiveGroup={Boolean(activeGroupId)} activeGroupName={activeGroupName} activeGroupMemberIds={activeGroupMemberIds} roster={roster} initialAttendees={attendees} />
          </div>
          <div className="mt-4">
            <AskGroupButton eventId={feedEvent.id} groupId={activeGroupId} groupName={activeGroupName} />
            <GroupAvailability eventId={feedEvent.id} groupId={activeGroupId} />
          </div>
        </section>

        {activeGroupId && (
          <section className="mt-7">
            <h2 className="font-display text-2xl font-bold text-zinc-900">Mommas&apos; tips</h2>
            <div className="mt-3"><TipsSection placeId={feedEvent.place_id ?? undefined} eventId={feedEvent.place_id ? undefined : feedEvent.id} groupId={activeGroupId} groupName={activeGroupName} currentUserId={user.id} tips={tips} /></div>
          </section>
        )}

        {activeGroupId && (
          <section className="mt-7">
            <h2 className="font-display text-2xl font-bold text-zinc-900">Conversation</h2>
            <div className="mt-3"><EventComments eventId={feedEvent.id} groupId={activeGroupId} currentUserId={user.id} currentUserName={myProfile?.display_name ?? "You"} initialComments={comments} roster={Object.fromEntries(Object.entries(roster).map(([memberId, member]) => [memberId, member.display_name]))} /></div>
          </section>
        )}
      </EventCardShell>
    </div>
