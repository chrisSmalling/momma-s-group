...
export default async function GroupsPage(props: PageProps<"/groups">) {
  ...
  // Which groups already have something on the calendar, so a quiet group
  // can get a Poppy nudge instead of just a member roster.
  const { data: upcomingGroupEvents } = groupIds.length
    ? await supabase.from("feed_events").select("proposed_by_group").in("proposed_by_group", groupIds).gte("starts_at", new Date().toISOString())
    : { data: [] };
  const groupsWithPlans = new Set((upcomingGroupEvents ?? []).map((e) => e.proposed_by_group).filter((id): id is string => Boolean(id)));
    ...
    <div className="flex flex-1 flex-col items-center px-4 py-6 sm:py-10">
      <div className="w-full max-w-2xl">
        ...
        <section aria-label="Your groups" className="flex flex-col gap-4">
          ...
          {groupList.map((group) => { const members = membersByGroup[group.id] ?? []; const me = members.find((m) => m.user_id === user.id); return <article key={group.id} className="rounded-3xl border border-zinc-200/80 bg-white p-4 shadow-sm sm:p-5"><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><h2 className="font-display text-xl font-bold text-zinc-950">{group.name}</h2><p className="mt-1 text-xs text-zinc-500">{members.length} member{members.length === 1 ? "" : "s"}</p></div><div className="flex flex-wrap items-center gap-2"><a href={`#invite-${group.id}`} className="inline-flex min-h-11 items-center rounded-xl bg-zinc-900 px-3 py-2 text-xs font-bold text-white">👥 Invite moms</a><div className="rounded-xl bg-zinc-50 px-3 py-2 text-left ring-1 ring-zinc-100"><div className="text-[10px] font-bold uppercase tracking-wider text-zinc-600">Invite code</div><div className="mt-0.5 font-mono text-sm font-bold tracking-wider text-zinc-800">{group.invite_code}</div></div><LeaveGroupButton groupId={group.id} groupName={group.name} /></div></div><div id={`invite-${group.id}`} className="mt-3"><GroupInvite groupName={group.name} inviteCode={group.invite_code} /></div><ul className="mt-4 divide-y divide-zinc-100 rounded-2xl border border-zinc-100" aria-label={`${group.name} members`}>{members.map((member) => <li key={member.user_id} className="flex items-start gap-3 px-3 py-3"><span aria-hidden="true" className="mt-1 h-3 w-3 shrink-0 rounded-full ring-2 ring-white" style={{ backgroundColor: member.avatar_color }} /><div className="min-w-0"><div className="text-sm font-semibold text-zinc-800">{member.display_name}{member.user_id === user.id && <span className="ml-1.5 font-medium text-zinc-600">(you)</span>}</div>{member.things_to_know && <p className="mt-0.5 text-xs leading-5 text-zinc-500">{member.things_to_know}</p>}</div></li>)}</ul>{!groupsWithPlans.has(group.id) && <div className="mt-4"><PoppyNudge heading={`Nothing planned for ${group.name} yet`} subtext="Ask Poppy for an idea you could propose to the group." ask="Something fun for a group to do this weekend" groupId={group.id} /></div>}<details className="mt-4 border-t border-zinc-100 pt-3"><summary className="flex min-h-11 cursor-pointer items-center justify-between text-sm font-bold text-zinc-600"><span>{me?.things_to_know ? "Edit your notes for this group" : "Add notes for this group"}</span><span aria-hidden="true">⌄</span></summary><p className="mb-2 text-xs text-zinc-500">Optional notes other group members may need to know.</p><form action={updateThingsToKnow} className="flex flex-col gap-2"><input type="hidden" name="group_id" value={group.id} /><textarea name="things_to_know" maxLength={300} rows={2} defaultValue={me?.things_to_know ?? ""} placeholder="Anything helpful for the group" className="w-full rounded-xl border border-zinc-200 px-3 py-2 text-sm outline-none focus:border-rose-400 focus:ring-2 focus:ring-rose-100" /><button type="submit" className="min-h-11 self-start rounded-xl bg-zinc-900 px-4 py-2 text-sm font-bold text-white">Save notes</button></form></details></article>; })}
        </section>

        <section aria-label="Group actions" className="mt-8 rounded-3xl border border-rose-100 bg-gradient-to-br from-rose-50 via-white to-amber-50 p-4 shadow-sm sm:p-5">
          <div className="mb-4"><h2 className="font-display text-lg font-bold text-zinc-950">Join or create another group</h2><p className="mt-1 text-xs text-zinc-500">Use an invite code if someone sent you one.</p></div>
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <form action={joinGroup} className="rounded-2xl border border-white bg-white/90 p-4 shadow-sm"><label className="text-sm font-bold text-zinc-800">Join a group</label><input name="code" required placeholder="Enter invite code" autoCapitalize="characters" className="mt-2 min-h-11 w-full rounded-xl border border-zinc-200 bg-white px-3 py-2 text-sm uppercase tracking-wider outline-none focus:border-rose-400 focus:ring-2 focus:ring-rose-100" /><button type="submit" className="mt-2 min-h-11 w-full rounded-xl border border-zinc-300 bg-white px-3 py-2 text-sm font-bold text-zinc-900">Join group</button></form>
          </div>
        </section>
      </div>
    </div>
