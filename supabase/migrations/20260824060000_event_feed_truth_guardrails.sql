...
create trigger trg_normalize_event_text_fields
before insert or update of title,description,venue_name,venue_display,organizer,room_name,address,cost,display_title
on public.events
for each row execute function public.normalize_event_text_fields();
...
create trigger trg_guard_recurring_event_occurrence
before insert or update of program_id,starts_at
on public.events
for each row execute function public.guard_recurring_event_occurrence();
...
begin
  if new.source_id is not null then
    select id,active,source_type,reliability_score,last_success_at,last_event_count into cs
    from public.content_sources where id=new.source_id;
    if coalesce(cs.active,false) and cs.source_type='structured_web'
       and coalesce(cs.reliability_score,0)>=95
       and coalesce(cs.last_event_count,0)>0
       and cs.last_success_at >= now()-interval '24 hours' then
...
create trigger trg_guard_source_verification
before insert or update of source_id,title,description,starts_at,ends_at,venue_name,address
on public.events
for each row execute function public.guard_source_verification();

update public.events
set title=public.normalize_event_text(title),
    description=public.normalize_event_text(description),
    venue_name=public.normalize_event_text(venue_name),
    venue_display=public.normalize_event_text(venue_display),
    organizer=public.normalize_event_text(organizer),
    room_name=public.normalize_event_text(room_name),
    address=public.normalize_event_text(address),
    cost=public.normalize_event_text(cost),
    display_title=public.normalize_event_text(display_title);

update public.events e
set is_suppressed=true,
    verification_reasons=coalesce(e.verification_reasons,'[]'::jsonb) || jsonb_build_array(jsonb_build_object('code','invalid_recurrence_occurrence','rrule',r.rrule))
from public.recurring_programs r
where e.program_id=r.id
  and e.status='published'
  and not e.is_suppressed
  and not public.recurrence_occurrence_matches(e.starts_at,r.rrule);

-- Confirmed closed venue: keep historical records but remove it from place suggestions.
update public.places
set active=false
where id='e97276c3-a3ca-4627-86a2-6f506db57766';

-- Feed contract: user-visible events require recent high/trusted verification.
...
select
  ...
  coalesce(e.lat,e.location_latitude) as lat,
  coalesce(e.lng,e.location_longitude) as lng,
  e.location_latitude,e.location_longitude,e.starts_at,e.ends_at,e.time_precision,
  (e.time_precision='date_only') as time_unknown,
  e.cost,
...
left join public.recurring_programs r on r.id=e.program_id
where e.status='published'
  and e.content_status='keep'
  and e.is_kid_relevant
  and not e.is_suppressed
  and e.duplicate_of is null
  and e.duplicate_of_event_id is null
  and e.verification_tier in ('trusted','high')
  and e.verification_score>=80
  and e.last_verified_at is not null
  and e.last_verified_at>=now()-interval '7 days'
...
select
  count(*) filter (where e.status='published' and e.starts_at>=now()) as upcoming_published,
  count(*) filter (where e.status='published' and e.starts_at>=now() and not e.is_suppressed and e.content_status='keep') as eligible_before_verification,
  count(*) filter (where e.status='published' and e.starts_at>=now() and not e.is_suppressed and e.content_status='keep' and (e.verification_tier not in ('trusted','high') or e.verification_score<80 or e.last_verified_at is null or e.last_verified_at<now()-interval '7 days')) as unsafe_records_blocked,
  count(*) filter (where e.status='published' and e.starts_at>=now() and not e.is_suppressed and e.program_id is not null and not public.recurrence_occurrence_matches(e.starts_at,r.rrule)) as invalid_recurrence_blocked,
  count(*) filter (where e.status='published' and e.starts_at>=now() and not e.is_suppressed and (e.description~*'<[^>]+>|&lt;|&gt;|\\\\1' or e.address~*'<[^>]+>|&lt;|&gt;|\\\\1')) as dirty_text_records
