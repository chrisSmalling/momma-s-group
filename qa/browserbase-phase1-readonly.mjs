...
async function visit(path, label) {
  ...
  if (!response || response.status() >= 400) {
    throw new InfrastructureFailure(`${label} failed before UI assertions: HTTP ${response ? response.status() : "no response"}`);
  }
  if (pathname === "/login") {
    throw new InfrastructureFailure(`${label} redirected to /login; QA session is no longer valid.`);
  }
...
try {
  ...
  assert("Explorer: query input present", await page.getByPlaceholder(/try .*toddler/i).count() > 0);
